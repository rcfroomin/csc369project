...
def data_classification(df : DataFrame): DataFrame = {
  ...
  i attempted a sort of merge, lmk what u think:

def prob_with_laplace(df: DataFrame): Unit = {
  val classificationColumns = df.columns.filter(_.startsWith("s_")) // Sensor columns starting with "s_"

  // Collect counts for each classification in each sensor column
  ...
  colCounts.foreach { case (sensor, counts) =>
    println(s"Counts for sensor $sensor: $counts")
  }
  ...
  colCounts.foreach { case (sensor, countsMap) =>
    val n_j = countsMap.values.sum // Total count for the column (all classifications combined)
    countsMap.foreach { case (classification, n_ij) =>
      val probability = (n_ij + lambda) / (n_j + lambda * m_i) // Apply Laplace formula
      println(s"Sensor: $sensor, Classification: $classification, Probability: $probability")
    }
  }
}
